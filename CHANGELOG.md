...
- `CONTRACT.md`: the thirteen rules that bind every session, identical in all three
  repositories . No INTERFACES change. Two files already cited it by rule number
  ...
  mean something else.
- `control.py`: the control corpus and the false-positive budget . INTERFACES change:
...
- `eval/harness.py`: `transfer` gained `controls=`, and `TransferResult` gained
  `fitted_detector` and `control_reports` . INTERFACES change: two new fields and one
...
- `notches.py`: the declarative notch catalogue . `NotchEntry`, `load_notches`,
...
- CI, a release procedure and the `submodule-parity` job .

## 0.1.0

First release. The library as built during scaffolding, plus the P1 work landed since.

- `inject.py`: the distortion injection harness . Four injectors returning an
...
- `workorders/`: card and ambiguity templates .
- CI .
