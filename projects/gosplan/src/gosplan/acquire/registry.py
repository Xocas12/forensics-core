...
registration-gated, reference-only or reference an archive a person must physically visit.
Those are reported as skipped with the reason from the registry, which is the honest outcome.
``data/ACCESS_NOTES.md`` says, per id, why there is no code for it.
"""
